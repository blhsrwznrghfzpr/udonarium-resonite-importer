...
describe('Converter integration (sample-card.zip)', () => {
  it('converts card and card-stack objects', async () => {
...
describe('Converter integration (sample-mapmask.zip)', () => {
  it('converts table-mask objects with alpha blend and thin collider', async () => {
    ...
    const masks = flattened.filter((obj) => {
      ...
      const colliderZ = (collider?.fields.Size as { value?: { z?: number } } | undefined)?.value?.z;
...
describe('Converter integration (sample-terrain.zip)', () => {
  it('converts terrain objects with top and walls child slots', async () => {
